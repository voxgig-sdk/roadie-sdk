...
[MCP](https://modelcontextprotocol.io) server exposing the Roadie SDK as
two agent tools — `roadie_list` and `roadie_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/roadie-mcp)
make build

# 2. Provide credentials via the environment
export ROADIE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user roadie \
  -- /absolute/path/to/roadie-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./roadie-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// roadie_list: first page of records
{ "entity": "entity" }
{ "entity": "entity", "query": { } }

// roadie_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/roadie-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user roadie \
     -- "$PWD"/dist/*/roadie-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `roadie_list` and `roadie_load` tools now appear
   in new sessions. Ask the agent to *"list entity using roadie"*
   and it calls `roadie_list` with `{"entity":"entity"}`.
...
```sh
export ROADIE_APIKEY=sk_live_xxx            # API key
export ROADIE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `roadie_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `roadie_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ROADIE_APIKEY` | API key sent with every request. |
| `ROADIE_BASE` | Optional override of the API base URL. |
...
```sh
./roadie-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"roadie_load","arguments":{"entity":"entity","query":{"id":1}}}}'
```
