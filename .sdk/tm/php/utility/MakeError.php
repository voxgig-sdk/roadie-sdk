...
// Roadie SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class RoadieMakeError
{
    public static function call(?RoadieContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RoadieContext([], null);
        }
        $op = $ctx->op ?? new RoadieOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RoadieResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RoadieError) ? $err->msg : (string)$err;
        $msg = "RoadieSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new RoadieError('', $msg, $ctx);
