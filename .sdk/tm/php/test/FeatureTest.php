...
class FtCtrl extends RoadieControl
{
...
class FtClient
{
    ...
    private RoadieUtility $utility;

    public function __construct(RoadieUtility $utility, array $options)
    {
    ...
    public function get_utility(): RoadieUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RoadieContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new RoadieUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (RoadieContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RoadieConfig::make_feature($name);
    ...
    private static function buildUrl(RoadieSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RoadieError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RoadieResponse($fetched);
        }
    ...
    private function fail(RoadieContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RoadieContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
