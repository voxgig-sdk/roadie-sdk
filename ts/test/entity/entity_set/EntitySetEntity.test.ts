...
describe('EntitySetEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'entity_set.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set ROADIE_TEST_ENTITY_SET_ENTID JSON to run live')
      return
    }
    ...
    let entity_set_ref01_data = Object.values(setup.data.existing.entity_set)[0] as any

    // LIST
    const entity_set_ref01_ent = client.EntitySet()
    const entity_set_ref01_match: any = {}

    const entity_set_ref01_list = (await entity_set_ref01_ent.list(entity_set_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ROADIE_TEST_ENTITY_SET_ENTID']
