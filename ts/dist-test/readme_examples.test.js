...
const TEST_SEED = { "entity": { "entity": { "test01": { "id": "test01" } }, "entity_set": { "test01": { "id": "test01" } }, "entity_set_push": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+RoadieSDK\b/.test(code) ||
        /\bRoadieSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+RoadieSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/RoadieSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
