...
class EntityEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EntityEntity($this->_client, $opts);
    }

    /**
     * @param Entity|array $args Entity data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RoadieHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Entity|array The current Entity data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Entity fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RoadieHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Entity fields).
     */
     ...
     *
     * @param EntityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EntityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entity|array The loaded Entity as an assoc-array at the
     *   SDK boundary; throws RoadieError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RoadieHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param EntityListMatch|array|null $reqmatch Match filter (any subset
     *   of Entity fields) as an assoc-array; EntityListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entity[]|array A list of Entity items as assoc-arrays at
     *   the SDK boundary; throws RoadieError on failure (item-5 convention).
     */
     ...
     * Create a new Entity.
     *
     * @param EntityCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EntityCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entity|array The created Entity as an assoc-array at the
     *   SDK boundary; throws RoadieError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Entity matching the given criteria.
     *
     * @param EntityRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; EntityRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Entity|array The removed Entity as an assoc-array at the
     *   SDK boundary; throws RoadieError on failure (item-5 convention).
     */
