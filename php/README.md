...
The PHP SDK for the Roadie API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Entity()` — with named operations (`list`/`load`/`create`/`update`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
$client = RoadieSDK::test($testopts, $sdkopts);
```
...
The main class (`roadie_sdk.php`) exports the SDK class
