...
// Roadie SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class RoadieMakePoint
{
    public static function call(RoadieContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof RoadieError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = RoadieHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = RoadieHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
