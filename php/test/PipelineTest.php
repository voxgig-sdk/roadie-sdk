...
{
    private static function utility(): RoadieUtility
    {
        return new RoadieUtility();
    }
    ...
    private static function ctx(array $over = []): RoadieContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RoadieContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RoadieOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RoadieError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RoadieOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RoadieMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RoadieOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RoadieMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RoadieOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RoadieMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RoadieSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RoadieMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RoadieResponse([]),
            'result' => new RoadieResult([]),
        ]);
        [, $err] = RoadieMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RoadieSpec(['step' => 's']),
            'response' => new RoadieResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RoadieResult([]),
        ]);
        [, $err] = RoadieMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RoadieSpec(['step' => 's']),
            'response' => new RoadieResponse(self::resp(200, ['v' => 1])),
            'result' => new RoadieResult([]),
        ]);
        [, $err] = RoadieMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RoadieSpec(['step' => 's']),
            'response' => new RoadieResponse(self::resp(200, ['v' => 2])),
            'result' => new RoadieResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RoadieMakeResponse::call($ctx);
    ...
    {
        $preset = new RoadieResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RoadieSpec([]),
            'response' => new RoadieResponse([]),
            'result' => new RoadieResult([]),
        ...
        [$got, $err] = RoadieMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RoadieResult([])]);
        [, $err] = RoadieMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RoadieSpec([]), 'result' => null]);
        [, $err] = RoadieMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RoadieSpec(['step' => 's']),
            'result' => new RoadieResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RoadieOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RoadieMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RoadieSpec(['step' => 's']),
            'result' => new RoadieResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RoadieOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RoadieMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RoadieResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RoadieSpec([]),
            'result' => new RoadieResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RoadieMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RoadieMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RoadieError('boom', 'boom');
        $utility->fetcher = function (RoadieContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RoadieSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RoadieMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RoadieContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RoadieMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RoadieResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RoadieContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RoadieSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RoadieMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RoadieResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RoadieSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RoadieMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RoadieMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RoadieMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RoadieResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RoadieDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RoadieResult(['ok' => false])]);
        $this->expectException(RoadieError::class);
        RoadieDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RoadieResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RoadieMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RoadieBaseFeature();
        $b = new RoadieBaseFeature();
        RoadieFeatureAdd::call($ctx, $a);
        RoadieFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RoadieBaseFeature
    {
        $f = new RoadieBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RoadieFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RoadieContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RoadieSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RoadiePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RoadieResponse(['headers' => null]),
            'result' => new RoadieResult([]),
        ]);
        RoadieResultHeaders::call($ctx);
