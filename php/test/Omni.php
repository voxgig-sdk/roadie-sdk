...
//    only - a typed RoadieContext there would make every ctx
...
final class RoadieOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RoadieContext($clean, null);
    ...
    public static function err_from_map($m): ?RoadieError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RoadieError(is_string($code) ? $code : '', $msg);
    }
