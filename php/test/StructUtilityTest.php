...
// Roadie SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RoadieOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RoadieOmni::structfix($now, Struct::undef()))
                        !== json_encode(RoadieOmni::structfix($val, Struct::undef()))) {
