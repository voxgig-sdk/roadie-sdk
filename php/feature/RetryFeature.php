...
class RoadieRetryFeature extends RoadieBaseFeature
{
    ...
    public function init(RoadieContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RoadieContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RoadieContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RoadieContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
