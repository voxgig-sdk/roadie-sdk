...
class RoadieSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = RoadieHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RoadieHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RoadieFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, RoadieFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return RoadieUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = RoadieHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = RoadieHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = RoadieHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RoadieError($op . "_allow",
                "RoadieSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = RoadieHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RoadieError("graphql_error",
                "RoadieSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Entity($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_entity === null) {
                $this->_entity = new EntityEntity($this, null);
            }
            return $this->_entity;
        }
        return new EntityEntity($this, $data);
    }
