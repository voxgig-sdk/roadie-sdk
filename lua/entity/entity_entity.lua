...
local EntityEntity = {}
EntityEntity.__index = EntityEntity


function EntityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EntityEntity)
  self._name = "entity"
  self._client = client
...
function EntityEntity:get_name()
  return self._name
end


function EntityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EntityEntity.new(self._client, opts)
end
...
function EntityEntity:mark_deleted()
  self._deleted = true
end


function EntityEntity:deleted()
  return true == self._deleted
end


function EntityEntity:data_set(args)
  if args ~= nil then
...
function EntityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EntityEntity:match_set(args)
  if args ~= nil then
...
function EntityEntity:match_get()
...
function EntityEntity:stream(action, args, callopts)
...
---@param reqmatch EntityLoadMatch
---@param ctrl? table
---@return Entity
---@return string? err
function EntityEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EntityListMatch
---@param ctrl? table
---@return Entity[]
---@return string? err
function EntityEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata EntityCreateData
---@param ctrl? table
---@return Entity
---@return string? err
function EntityEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch EntityRemoveMatch
---@param ctrl? table
---@return Entity
---@return string? err
function EntityEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EntityEntity:_run_op(ctx, post_done)
  local utility = self._utility
