...
func TestEntitySetEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.EntitySet(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.EntitySet(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		entitySetRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.entity_set", setup.data)))
