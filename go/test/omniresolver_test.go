...
import (
	...
	omni "github.com/voxgig-sdk/roadie-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RoadieError); is {
				out := map[string]any{
					"name":    "RoadieError",
...
func omniCtx(arg any, client *sdk.RoadieSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RoadieSDK); is {
			client = live
