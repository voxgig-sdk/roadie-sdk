...
func TestEntityDirect(t *testing.T) {
	t.Run("direct-list-entity", func(t *testing.T) {
		setup := entityDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-entity", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-entity", func(t *testing.T) {
		setup := entityDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-entity", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "api/catalog/roadie-entities/entities/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type entityDirectSetupResult struct {
...
func entityDirectSetup(mockres any) *entityDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ROADIE_TEST_ENTITY_ENTID"]; ok {
		...
		return &entityDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &entityDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
