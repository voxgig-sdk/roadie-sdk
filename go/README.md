...
The Golang SDK for the Roadie API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Entity(nil)` — each with the same small set of operations (`List`, `Load`, `Create`, `Update`, `Remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/roadie-sdk/go@latest
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/roadie-sdk/go=../roadie-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *RoadieSDK
```
...
All entities implement the `RoadieEntity` interface.
...
Create an instance: `entity := client.Entity(nil)`
...
| `set` | `string` |  |
| `source` | `string` |  |
| `spec` | `map[string]any` | Kind-specific fields. |
| `updatedAt` | `string` |  |
| `updatedBy` | `string` |  |
...
```
github.com/voxgig-sdk/roadie-sdk/go/
├── roadie.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/roadie-sdk/go`) re-exports everything needed
