...
boru-driven command-line client **and** interactive REPL for the Roadie
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/roadie-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/roadie-cli
   ```
...
```sh
./roadie-cli load 1 entity          # scalar shorthand for {id:1}
./roadie-cli load '{id:1}' entity     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `roadie>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `entity`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/roadie-cli`. |
...
(`./roadie-cli <expr>`) and interactively (the REPL), and expressions compose the
