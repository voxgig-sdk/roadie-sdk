// Command Roadie-cli is a CLI + REPL for the Roadie SDK,
...
const entitiesHelp = "entity entity_set entity_set_push"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ROADIE_APIKEY (api key), ROADIE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ROADIE_APIKEY carries the API key and
	// ROADIE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ROADIE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ROADIE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRoadieSDK(opts)
