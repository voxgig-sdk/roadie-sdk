    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ROADIE_TEST_ENTITY_ENTID JSON to run live")
        client = setup["client"]
        ...
        entity_ref01_data = helpers.to_map(runner.entity_data(entity_ref01_ent.create(entity_ref01_data, None)))
        assert entity_ref01_data is not None
        assert entity_ref01_data["id"] is not None
        ...
        entity_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(entity_ref01_data_dt0_loaded))
        ...
        entity_ref01_ent.remove(entity_ref01_match_rm0, None)

        # LIST
        entity_ref01_match_rt0 = {}

        entity_ref01_list_rt0_result = entity_ref01_ent.list(entity_ref01_match_rt0, None)
        assert isinstance(entity_ref01_list_rt0_result, list)
...
def _entity_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ROADIE_TEST_ENTITY_ENTID"))
    ...
    if env.get("ROADIE_TEST_LIVE") == "TRUE":
        ...
        client = RoadieSDK(helpers.to_map(merged_opts))

    _live = env.get("ROADIE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ROADIE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
